// componenets/map/map-search.tsx
...
import {
  ...
  import { setSelectedLocation,setSelectedLocations } from "@/redux/map/mapSlice";
...
export default function MapSearch() {
  ...
  const selectedLocation=useSelector((state:any)=>state.map.selectedLocation)
const selectedLocations=useSelector((state:any)=>state.map.selectedLocations)
      ...
      try {
        const res = await fetch(
          `https://api.mapbox.com/search/searchbox/v1/suggest?q=${encodeURIComponent(
            ...
            import.meta.env.VITE_PUBLIC_MAPBOX_SESSION_TOKEN
          }&country=ML&limit=5&proximity=-7.985,12.6392&bbox=-12.5,10.0,-4.0,25.0`
        );
    ...
    try {
      ...
      const res = await fetch(
        `https://api.mapbox.com/search/searchbox/v1/retrieve/${
          ...
          import.meta.env.VITE_PUBLIC_MAPBOX_TOKEN
        }&session_token=${import.meta.env.VITE_PUBLIC_MAPBOX_SESSION_TOKEN}`
      );
      ...
      if (map && featuresData?.length > 0) {
        ...
        dispatch(setSelectedLocations(featuresData));
        dispatch(setSelectedLocation(featuresData[0]));
  ...
  return (
    <>
      <section className="absolute top-5 sm:top-10 left-1/6 sm:left-4 z-10 w-[30vw] sm:w-[350px] -translate-x-1/2 sm:translate-x-0 rounded-lg shadow-lg">

...
import { useDeleteCellPvComponentMutation } from "@/redux/componentsManageApi/cellPVComponent/cellPVComponentApi";
import { Button } from "@/components/ui/button";

export default function DeletePvModuleModal({
  open,
  setOpen,
  idComponent,
}: {
  ...
  const [deleteCellPvComponent, { isLoading }] =
    useDeleteCellPvComponentMutation();

  //   fonction pour supprimer un composant
  const onDelete = async () => {
    toast.promise(deleteCellPvComponent(idComponent).unwrap(), {
      loading: "Suppression en cours...",
      error: (error: any) => {
        console.error("Error:delete cell pv ", error);
        if (error) {
      ...
      success: () => {
        setOpen(false);
        return "Composant cellule PV supprimé avec success";
      },
    });
  };
        ...
        >
          <DialogHeader className="space-y-3 p-3 pb-4 border-b border-zinc-100 dark:border-zinc-800 text-center">
            <DialogTitle className="text-xl font-semibold text-red-600 dark:text-red-400">
              Supprimer une cellule PV
            </DialogTitle>
            <DialogDescription className="text-sm text-zinc-600 dark:text-zinc-400">
              Êtes-vous sûr de vouloir supprimer ce module photovoltaïque ?
              Cette action est{" "}
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          ...
          <DrawerDescription>
            Êtes-vous sûr de vouloir supprimer ce module photovoltaïque ? Cette

...
} from "@/redux/componentsManageApi/inverterComponent/inverterComponentApi";
...
export const PVForm = ({
  ...
  const [updateInverterComponent, { isLoading }] =
    useUpdateInverterComponentMutation();

  const [getInverterComponent] = useLazyGetInverterComponentQuery();
  ...
  const onSubmit = async (data: TableData) => {
    toast.promise(updateInverterComponent({ ...data }).unwrap(), {
      loading: "Edition en cours...",
      error: (error: any) => {
        console.error("Error:edit inverter pv ", error);
        if (error) {
          const error_custom: any = error;
          return (
            error_custom?.data?.message ||
            "Erreur lors de la modification du composant"
          );
        }
      },
      success: () => {
        setOpen(false);
        return "Composant onduleur modifié avec success";
      },
    });
  };

  //   fonction de récupération des infos du composant
  const fetchData = async () => {
    try {
      const res = await getInverterComponent(idComponent);
        reset(res.data.data);
    } catch (error) {
      console.log("erreur:get inverter by id ", error);
      if (error) {
        const error_custom: any = error;
        toast.error(
          error_custom?.data?.message ||
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end pt-6 gap-4 border-t border-zinc-100 dark:border-zinc-800">
        ...
        >
          {isLoading ? "Modification en cours..." : "Modifier l'onduleur"}
        </Button>
      </div>
    </form>
  );
...
export default function EditInverterModal({
  open,
  setOpen,
  idComponent,
}: Props) {
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle> Modifier un Onduleur</DrawerTitle>
          <DrawerDescription>
            Configurez les spécifications techniques de votre onduleur
          </DrawerDescription>
        </DrawerHeader>

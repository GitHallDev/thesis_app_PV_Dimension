...
} from "@/redux/componentsManageApi/cellPVComponent/cellPVComponentApi";
...
export const PVForm = ({
  ...
  const [updateCellPvComponent, { isLoading }] =
    useUpdateCellPvComponentMutation();

  const [getCellPvComponent] = useLazyGetCellPvComponentQuery();
  ...
  const onSubmit = async (data: TableData) => {
    toast.promise(updateCellPvComponent({ ...data }).unwrap(), {
      loading: "Edition en cours...",
      error: (error: any) => {
        console.error("Error:edit cell pv ", error);
        if (error) {
      ...
      success: () => {
        setOpen(false);
        return "Composant cellule PV modifié avec success";
      },
    });
  };
  ...
  const fetchData = async () => {
    try {
      const res = await getCellPvComponent(idComponent);
      //   console.log("data recived: ", res.data.data);
      reset(res.data.data);
    } catch (error) {
      console.log("erreur:get PV Cell by id ", error);
      if (error) {
        const error_custom: any = error;
        toast.error(
          error_custom?.data?.message ||
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end pt-6 gap-4 border-t border-zinc-100 dark:border-zinc-800">
        ...
        >
          {isLoading ? "Modification en cours..." : "Modifier le module"}
        </Button>
      </div>
    </form>
  );
...
export default function EditPvModuleModal({
  open,
  setOpen,
  idComponent,
}: Props) {
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle> Modifier une cellule PV</DrawerTitle>
          <DrawerDescription>
            Configurez les spécifications techniques de votre module
            photovoltaïque
          </DrawerDescription>

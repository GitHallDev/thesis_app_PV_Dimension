...
import { useDeleteInverterComponentMutation } from "@/redux/componentsManageApi/inverterComponent/inverterComponentApi";
import { Button } from "@/components/ui/button";

export default function DeleteInverterModal({
  open,
  setOpen,
  idComponent,
}: {
  ...
  const [deleteInverterComponent, { isLoading }] =
    useDeleteInverterComponentMutation();

  //   fonction pour supprimer un composant
  const onDelete = async () => {
    toast.promise(deleteInverterComponent(idComponent).unwrap(), {
      loading: "Suppression en cours...",
      error: (error: any) => {
        console.error("Error:delete inverter pv ", error);
        if (error) {
      ...
      success: () => {
        setOpen(false);
        return "Composant onduleur supprimé avec success";
      },
    });
  };
        ...
        >
          <DialogHeader className="space-y-3 p-3 pb-4 border-b border-zinc-100 dark:border-zinc-800 text-center">
            ...
            <DialogDescription className="text-sm text-zinc-600 dark:text-zinc-400">
              Êtes-vous sûr de vouloir supprimer cet onduleur ? Cette action est{" "}
